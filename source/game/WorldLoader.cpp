...
bool WorldLoader::LoadWorld(void)
{
	...
	while (region_node)
	{
		...
		const float popul = region_node.attribute("Population").as_float();
...
void WorldLoader::_InitHullFromXml(const char *name, pugi::xml_node &root,  ArbitraryHull &hull) const
{
