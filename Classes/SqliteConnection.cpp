...
SqliteConnection::SqliteConnection(const std::string& databaseFile)
{
	...
	{
		WRITE_LOG(std::string("Database file isn't exist (creating): ").append(databaseFile));
	}
	...
	if (rc != 0)
	{
		WRITE_ERR(std::string("Can't open database: ").append(sqlite3_errmsg(_database)));
	}
}
...
void SqliteConnection::execSql(const std::string& statement)
{
	...
	{
		WRITE_ERR(std::string("Unable to execute SQL statement \"").append(statement)
