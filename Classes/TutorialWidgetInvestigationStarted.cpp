...
{
    TutorialWidgetInvestigationStarted* ret = new TutorialWidgetInvestigationStarted(tutorial);
    if (ret && ret->init())
    {
        ret->autorelease();
    }
...
bool TutorialWidgetInvestigationStarted::init()
{
    ...
    if (btnContinue)
    {
        btnContinue->addTouchEventListener(CC_CALLBACK_2(TutorialWidgetInvestigationStarted::OnContinueCallback, this));
