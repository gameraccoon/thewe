...
{
    TutorialWidgetInvestigatorUncatched* ret = new TutorialWidgetInvestigatorUncatched(tutorial);
    if (ret && ret->init())
    {
        ret->autorelease();
    }
...
bool TutorialWidgetInvestigatorUncatched::init()
{
    ...
    if (btnContinue)
    {
        btnContinue->addTouchEventListener(CC_CALLBACK_2(TutorialWidgetInvestigatorUncatched::OnContinueCallback, this));
