...
SqliteDataReader::SqliteDataReader(const std::string& query, sqlite3* db)
{
	...
	if (rc != 0)
	{
		WRITE_ERR(std::string("Unable to execute SQL query \"").append(query)
...
{	
	...
	WRITE_ERR("SqliteDataReader error: Column not found: " + columnName);
	return nullptr;
}
...
{	
	...
	{
		WRITE_ERR("SqliteDataReader error: Column not found: " + std::to_string(columnIndex));
	}
