...
bool MapGuiLayer::init(void)
{
	...
	Vector2 pos(origin.x + 100.0f, origin.y + screen.y - 100.0f);

	_printTime = cocos2d::CCLabelTTF::create("Time: 0", "Arial", 32);
	_printTime->setPosition(Vector2(origin.x + 100, origin.y + screen.y - 50));
	addChild(_printTime);

	_btnZoomIn->setScale(2.0f);
	_btnZoomIn->setTag(MENU_ITEM_ZOOM_IN);
	_btnZoomIn->setPosition(pos - Vector2(0.0f, 100.0f));
	_btnZoomOut->setScale(2.0f);
	_btnZoomOut->setTag(MENU_ITEM_ZOOM_OUT);
	_btnZoomOut->setPosition(pos - Vector2(0.0f, 200.0f));
	_btnEditor->setScale(2.0f);
	_btnEditor->setTag(MENU_ITEM_EDITOR);
	_btnEditor->setPosition(pos - Vector2(0.0f, 300.0f));
	_btnSave->setScale(2.0f);
	_btnSave->setTag(MENU_ITEM_SAVE);
	_btnSave->setPosition(pos - Vector2(0.0f, 400.0f));
