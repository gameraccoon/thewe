...
TutorialWidgetAfterFirstTask::TutorialWidgetAfterFirstTask(Tutorial::WeakPtr tutorial)
	: TutorialWidget(tutorial)
{
}

TutorialWidgetAfterFirstTask* TutorialWidgetAfterFirstTask::create(Tutorial::WeakPtr tutorial)
{
	TutorialWidgetAfterFirstTask* ret = new TutorialWidgetAfterFirstTask(tutorial);
	if (ret && ret->init())
	{
		ret->autorelease();
	}
...
bool TutorialWidgetAfterFirstTask::init()
{
	...
	if (btnContinue)
	{
		btnContinue->addTouchEventListener(CC_CALLBACK_2(TutorialWidgetAfterFirstTask::OnContinueCallback, this));
